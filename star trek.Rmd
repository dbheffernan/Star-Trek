...
kable(tbg%>%group_by(series_name)%>%summarise(IMDB_Episode_Average = mean(demo_adj),mean_standard_dev = sd(demo_adj))%>%arrange(desc(IMDB_Episode_Average)))
```
...
```{r}
seasons <- tbg %>% group_by(series_name,seasonNumber) %>% summarise(season_mean = mean(demo_adj))
ggplot(data=seasons, aes(x=seasonNumber, y=season_mean, group=series_name,colour = factor(series_name))) +
  geom_line()+
  geom_point()
```
...
```{r}
pairwise.ttest <- pairwise.t.test(x = tbg$demo_adj, g = tbg$series_name,pool.sd = FALSE)
test <- tbl_df(pairwise.ttest$p.value)
```
