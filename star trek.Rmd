...
story_By <- crew_scrape %>% 
  ...
  unnest()%>%
  to_dummy(var.name = 'story_by',suffix='label')
teleplay_By <- crew_scrape %>% 
  ...
  unnest()%>%
  to_dummy(var.name = 'teleplay_by',suffix='label')
...
e_se<- crew_scrape%>%
  ...
  unnest()%>%
  to_dummy(var.name = 'executive_story_editor',suffix='label')
...
written_By <- crew_scrape %>% 
  ...
  select(written_by)%>%
  unnest()%>%
  to_dummy(var.name = 'written_by',suffix='label')
...
staff_Writer <- crew_scrape %>% 
  ...
  unnest()%>%
  to_dummy(var.name = 'staff_writer',suffix='label')
...
story_Editor <- crew_scrape %>%
  ...
  unnest()%>%
  to_dummy(var.name = 'story_editor',suffix='label')
...
based_On <- crew_scrape %>%
 ...
 select(based_on)%>%
  ...
  unnest()%>%
  to_dummy(var.name = 'based_on',suffix='label')

created_By <- crew_scrape %>%
  mutate(created_by = str_replace_all(created_by, 'NA,+|NA,$|,$|,,NA|NA$', ''))%>%
  mutate(created_by = str_split(created_by, ","))%>%
  unnest()%>%
  to_dummy(var.name = 'created_by',suffix='label')
...
```{r}
  written_by <- crew$written_by %>% 
  mutate(written_by = str_split(written_by, ","))%>%
  unnest()%>%
  to_dummy(var.name = 'written',suffix='label')
  mutate(story_by = str_split(story_by, ","))%>%
  unnest()%>%  
  mutate(teleplay_by = str_split(teleplay_by, ","))%>%
  unnest()%>%  
...
spew <- tbg %>% select(tconst,demo_adj) %>% right_join(crew2,by='tconst')%>% select(-tconst)
```
