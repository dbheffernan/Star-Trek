...
crew <- crew_scrape %>% 
  ...
  unite('executive_story_editor', `executive story consultant`,
        ...
        sep = ',')%>%
  mutate(executive_story_editor = str_replace_all(
    executive_story_editor, 'NA$', ''))%>%
  ...
  unite('story_editor', `story editor`,`story editor and`,`story editor as David Bennett Carren`,sep=',')%>%
  ...
  mutate(story_by = str_replace_all(story_by, ',$|,,+', ''))%>%
  mutate(written_by = str_replace_all(`written by`, ',$|,,+', ''))%>%
  mutate(executive_story_editor = str_replace_all(
    executive_story_editor, ',$|,,+', ''))%>%
  mutate(staff_writer = str_replace_all(staff_writer, ',$|,,+', ''))%>%
  mutate(story_editor = str_replace_all(story_editor, ',$|,,+', ''))%>%
  mutate(based_on = str_replace_all(based_on, ',$|,,+', ''))%>%
  mutate(created_by = `created by`)%>%
  select(-`written by`,-`created by`)%>%
...
```{r}
  ...
  unnest()%>%
  mutate(story_by = strsplit(as.character(story_by), ","))%>%
  unnest()%>%  
  mutate(teleplay_by = strsplit(as.character(teleplay_by), ","))%>%
  unnest()%>%  
  mutate(director = strsplit(as.character(director), ","))%>%
  unnest()%>%
  mutate(showrunner = strsplit(as.character(showrunner), ","))%>%
  unnest()%>%  
  mutate(executive_story_editor = strsplit(as.character(executive_story_editor), ","))%>%
  unnest()%>%  
...
spew <- tbg %>% select(tconst,demo_adj) %>% right_join(crew,by='tconst')%>% select(-tconst)
```
